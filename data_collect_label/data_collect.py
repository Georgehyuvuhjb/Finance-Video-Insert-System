...
class PixabayVideoDownloader:
    ...
    def search_videos(self):
        ...
        # Minimum 3, maximum 200 per page
        videos_per_page = min(200, max(3, self.num_videos))
        pages_needed = (self.num_videos + videos_per_page -
                        1) // videos_per_page
    ...
    def load_metadata_database(self):
        ...
        if os.path.exists(self.metadata_db_path):
            try:
                ...
                print(
                    f"Warning: Could not read or parse {self.metadata_db_path}. A new file will be created.")
                return {}
        return {}
    ...
    def save_metadata_database(self, database):
        ...
        try:
            ...
            print(
                f"Successfully saved updated metadata to {self.metadata_db_path}")
        except IOError as e:
            print(
                f"Error: Could not write to {self.metadata_db_path}. Reason: {e}")
        ...
        for i, video_data in enumerate(videos):
            try:
                ...
                if str(video_id) in metadata_database:
                    print(
                        f"Video {i+1}/{len(videos)} (ID: {video_id}) already in database. Skipping download.")
                    continue
                ...
                if not video_url:
                    print(
                        f"No downloadable URL found for video {i+1} (ID: {video_id})")
                    continue
                ...
                print(
                    f"Downloading video {i+1}/{len(videos)}: {filename} ({video_size / (1024 * 1024):.2f} MB)")
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Failed to download video {i+1}. Status code: {response.status_code}")
                ...
                print(
                    f"An error occurred while processing video {i+1}: {str(e)}")
    ...
    def run(self):
        ...
        """
        print(
            f"Starting Pixabay API video downloader for '{self.search_term}' videos...")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Download videos from Pixabay and store metadata in a central JSON file.")
    parser.add_argument(
        "search_term", help="Search term for videos (e.g., 'finance+business')")
