...
class FFmpegCaptionGenerator:
    ...
    def check_available_encoders(self) -> List[str]:
        ...
        available_encoders = []
        encoders_to_check = ['libx264', 'libopenh264', 'mpeg4', 'libxvid', 'libvpx']
    ...
    def create_srt_file(self) -> str:
        ...
        for i, segment in enumerate(self.subtitle_segments, 1):
            ...
            # Write SRT entry
            temp_file.write(f"{i}\n")
            temp_file.write(f"{format_srt_time(segment.start_time)} --> {format_srt_time(segment.end_time)}\n")
    ...
    def build_ffmpeg_command(self, srt_file: str) -> List[str]:
        ...
        srt_path = srt_file.replace('\\', '/').replace(':', '\\:')  # Escape for FFmpeg
        ...
        if self.quality == "high":
            ...
            if 'libx264' in self.available_encoders:
                ...
                cmd.extend(['-c:v', 'libopenh264', '-crf', '23'])  # libopenh264 doesn't support crf 18
