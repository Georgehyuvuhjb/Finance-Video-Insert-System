...
def parse_size(size_str, main_width, main_height, original_width, original_height):
    ...
    # Handle absolute size (fallback)
        ...
        try:
            ...
            print(
                f"Could not parse size '{size_str}', using default (25% of width)")
            target_width = int(main_width * 0.25)
            target_height = int(target_width / aspect_ratio)
            return target_width, target_height
...
def run_ffmpeg_command(cmd, description="FFmpeg operation", show_output=False):
    ...
    try:
        ...
        if e.stderr:
            print(
                f"Error details: {e.stderr.decode('utf-8', errors='replace')}")
        return False
...
class VideoComposer:
    ...
    def __init__(self, json_input, transcript_file, output_file="composed_video.mp4",
        ...
        print(
            f"Loaded {len(self.matched_data)} sentence groups and {len(self.transcript)} transcript entries")
        ...
        for group in tqdm(self.matched_data, desc="Downloading videos"):
            ...
            if "matching_videos" in group:
                ...
                sentence_group = {
                    ...
                    "start_index": group.get("group_id", 0) - 1,
            ...
            eligible_videos = [
                v for v in videos if v["distance"] < self.distance_threshold]

            if not eligible_videos:
                print(
                    f"No eligible videos found for sentence: {sentence_group['sentences'][0][:30]}...")
                continue
                ...
                try:
                    ...
                    if not download_success:
                        try:
                            ...
                            print(
                                f"All download methods failed for {video_id}: {e}")
                            continue
            ...
            try:
                ...
                if os.path.exists(local_path):
                    ...
                    try:
                        ...
                        print(
                            f"Fallback probe failed for video {video_id}: {probe_err}")
    ...
    def match_segments_to_transcript(self, videos):
        ...
        for transcript_entry in self.transcript:
            ...
            if matching_video:
                ...
                if self.debug:
                    print(
                        f"Matched transcript: '{transcript_text[:30]}...' -> Video {matching_video['video_id']}")
            else:
                ...
                if best_match:
                    ...
                    if self.debug:
                        print(
                            f"Fuzzy matched transcript: '{transcript_text[:30]}...' -> Video {best_match['video_id']}")
                else:
                    print(
                        f"Warning: No matching video found for transcript: '{transcript_text[:50]}...'")

        print(
            f"Successfully matched {len(matched_segments)} transcript entries to videos")
    ...
    def compose_video(self, input_video, matched_segments):
        ...
        print(
            f"Loading input video: {input_video} ({main_width}x{main_height}, {main_fps} fps)")
        ...
        for match in tqdm(matched_segments, desc="Composing video"):
            ...
            try:
                # Get the clip
                clip_duration = min(end_time - start_time,
                                    video_data["duration"])
                if clip_duration <= 0:
                ...
                segment_clip = VideoFileClip(
                    video_path).subclip(0, clip_duration)
                ...
                # Resize the clip
                segment_clip = segment_clip.resize(
                ...
                # Set the time range
                segment_clip = segment_clip.set_start(
                    start_time).set_duration(clip_duration)

                # Add to composed clips
                composed_clips.append(segment_clip)

                print(
                    f"Added video clip at {start_time:.2f}-{end_time:.2f}s, position {x},{y}, size {target_width}x{target_height}")
        ...
        if self.audio_file and os.path.exists(self.audio_file):
            ...
            try:
                ...
                final_clip = final_clip.set_audio(
                    audio_clip.subclip(0, final_clip.duration))
...
def main():
    parser = argparse.ArgumentParser(
        description='Compose video from matched segments and input video')
    parser.add_argument('--json', required=True,
                        help='Path to JSON file with matched videos')
    parser.add_argument('--transcript', required=True,
                        help='Path to transcript file')
    parser.add_argument('--input-video', required=True,
                        help='Path to input video')
    parser.add_argument(
        '--output', default='composed_video.mp4', help='Path to output video')
    ...
    parser.add_argument(
        '--audio', help='Path to audio file to add to the final video')
