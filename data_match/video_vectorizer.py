...
def create_video_vectors(metadata_path, output_dir="outputs/vectors", model_name="paraphrase-multilingual-MiniLM-L12-v2", batch_size=32):
    ...
    for i in range(0, len(video_texts), batch_size):
        ...
        print(f"Processed batch {i//batch_size + 1}/{(len(video_texts)-1)//batch_size + 1}")
    ...
    faiss.write_index(index_flat, os.path.join(output_dir, "video_index_flat.faiss"))
    print(f"Saved flat index to {os.path.join(output_dir, 'video_index_flat.faiss')}")
    ...
    index_hnsw.hnsw.efConstruction = 200  # Higher value = more accurate but slower construction
    index_hnsw.add(video_embeddings)
    
    faiss.write_index(index_hnsw, os.path.join(output_dir, "video_index_hnsw.faiss"))
    print(f"Saved HNSW index to {os.path.join(output_dir, 'video_index_hnsw.faiss')}")
    ...
    print(f"Vector computation and indexing completed. All files saved to {output_dir}/")
    return id_mapping
...
def test_vector_search(output_dir="outputs/vectors", query="financial market stock trading", top_k=5):
    ...
    # Load the index
    index = faiss.read_index(os.path.join(output_dir, "video_index_flat.faiss"))
...
def main():
    parser = argparse.ArgumentParser(description='Create and save vector representations for videos')
    parser.add_argument('--metadata', default='outputs/video_metadata.json', help='Path to video metadata JSON file')
    parser.add_argument('--output', default='outputs/vectors', help='Output directory for vectors and index')
    parser.add_argument('--model', default='paraphrase-multilingual-MiniLM-L12-v2', help='Name of the sentence transformer model')
    parser.add_argument('--batch-size', type=int, default=32, help='Batch size for computing embeddings')
    parser.add_argument('--test', action='store_true', help='Run a test query after creating vectors')
    parser.add_argument('--test-query', default='financial market stock trading', help='Query to use for testing')
    
    args = parser.parse_args()
    
    id_mapping = create_video_vectors(args.metadata, args.output, args.model, args.batch_size)
